...
pub async fn change_password(
    ...
    sqlx::query! {
        r#"UPDATE users SET password_hash = $1 WHERE user_id = $2"#, password_hash.expose_secret(), user_id
    }.execute(pool).await.context("Failed to change user's password in database.")?;
    Ok(())
}
